...
export default function ServiceCard({ service, delay = 0 }: { service: Service; delay?: number }) {
  ...
  const name = lang === "en" && service.nameEn ? service.nameEn : service.name;
  const description = lang === "en" && service.descriptionEn ? service.descriptionEn : service.description;
  const deliverables = lang === "en" && service.deliverablesEn ? service.deliverablesEn : service.deliverables;
  ...
  return (
    <div style={{ perspective: 900 }}>
      ...
      >
        ...
        <h3 className="font-display font-semibold text-lg text-mist-100 mb-1">{name}</h3>
        <p className="text-2xl font-display font-bold text-gold-400 mb-4">{service.price}</p>
        <p className="text-sm text-mist-400 leading-relaxed mb-6">{description}</p>
        ...
        <a
          href={`https://wa.me/${contact.whatsapp}?text=${encodeURIComponent(
            lang === "en"
              ? `Hi Najwan, I am interested in the "${name}" package`
              : `Halo Najwan, saya tertarik dengan paket "${name}"`
          )}`}
          target="_blank"
